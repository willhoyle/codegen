...
local JSFile = {}
JSFile.__index = JSFile


function JSFile.new(filepath, opts)
  local defaults = {
    filepath = filepath
  }
  opts = opts or {}
  ...
  return setmetatable(vim.tbl_deep_extend("force", defaults, opts), JSFile)
end

function JSFile:exists()
...
-- function JSFile:marker(marker, marker_template)
...
-- function JSFile:insert_after_node(node, fragment_or_lines)
...
function JSFile:append(fragment_or_tbl)
...
function JSFile:init(fragment_or_tbl)
  if self:is_empty() then
...
-- function JSFile:insert_lines_precise(lines, row, col, indent, prepend_newline)
--   indent = indent or 0
...
-- function JSFile:tstree()
--   local parser = vim.treesitter.get_parser(self.bufnr, LANG)
--   local tstree = parser:parse()
--   return tstree
-- end

-- function JSFile:insert_imports(imports)
...
-- function JSFile:list_append(list_name, fragment_or_lines)
--   fragment_or_lines = fragment_or_lines or {}
--   local lines = fragment_or_lines.lines or fragment_or_lines
...
-- function JSFile:list_prepend(list_name, fragment_or_lines)
--   fragment_or_lines = fragment_or_lines or {}
--   local lines = fragment_or_lines.lines or fragment_or_lines
--   local list_query = template.render_string(
--     list_query_template,
...
-- function JSFile:last_class_variable(class_name)
...
-- function JSFile:class_identifier(class_name)
...
-- function JSFile:class_block(class_name)
...
-- function JSFile:class_variable_append(class_name, fragment_or_lines)
--   fragment_or_lines = fragment_or_lines or {}
--   local lines = fragment_or_lines.lines or { fragment_or_lines }
